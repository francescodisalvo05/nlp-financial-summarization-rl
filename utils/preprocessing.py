...
def get_tokens_to_filter(corpus_path, PATH_FILTERED_TOKENS, split):
    ...
    if split == 'train':
        ...
        # min_threshold = np.quantile(list(bow.values()), 0.50)
        # max_threshold = np.quantile(list(bow.values()), 0.975)
        # tokens_to_filter = [token for token, count in dict(bow).items() if count >= min_threshold]
