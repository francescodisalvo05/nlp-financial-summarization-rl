...
@curry
def compute_bert_score(output, reference, bertscore=None):
    """ compute bert score for a single pair of summary and reference
    output, reference
    """
    output_sent = ' '.join(output)
    output_sent = output_sent.replace("<sos>","").replace("<eos>","")

    reference_sent = ' '.join(reference)
    reference_sent = reference_sent.replace("<sos>","").replace("<eos>","")

    score = bertscore.compute(predictions = [output_sent], references = [reference_sent], lang="en")
    result = score["f1"][0]
    return result
